...
### Setup
```bash
# Clean and build the project to ensure dependencies are resolved
...
./gradlew test --tests "com.his.service.impl.DoctorServiceImplTest"

# Format code (if using a plugin like Spotless)
./gradlew spotlessApply
```

### Before Committing
```bash
# Run full build including tests and checks
./gradlew build
```
...
### Unit Testing
- Every module must have corresponding tests using JUnit 5 and Mockito.
- Use `@ExtendWith(MockitoExtension.class)` for unit tests.
- Mock external dependencies (repositories, other services).
- Target: >80% coverage for new business logic.
